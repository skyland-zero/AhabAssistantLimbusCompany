...
/// Fixed theme-skin artwork shipped with the GPUI client.
///
/// Unlike game screenshots, every file here is procedurally generated by
/// `scripts/generate_limbus_theme.py`, so the set is safe to embed and
/// redistribute under the repository license.
#[derive(Clone, Copy, Debug, Eq, Hash, PartialEq)]
pub enum ThemeAsset {
...
pub fn theme(asset: ThemeAsset) -> Asset {
    Asset::Theme(asset)
}
...
mod tests {
    ...
    fn all_canonical_assets_are_embedded_and_have_relative_paths() {
        ...
        for theme in ThemeAsset::ALL {
            let asset = Asset::Theme(theme);
            assert!(!asset.embedded().is_empty());
            assert!(!asset.relative_path().is_absolute());
            assert_eq!(
                asset.relative_path().parent().and_then(|p| p.file_name()),
                Some(std::ffi::OsStr::new("limbus"))
            );
        }
    }
