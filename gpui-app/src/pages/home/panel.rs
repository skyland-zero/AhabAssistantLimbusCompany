    ...
    fn sync_screenshot_image(&mut self, window: &mut Window, cx: &mut Context<Self>) {
            ...
            && let Some(source) = self.screenshot_pending_image_source.take()
        {
            if let Some(render_image) = source.clone().use_render_image(window, cx) {
                // The pinned GPUI/DirectX path renders the preview with its
                // red and blue components reversed when reused directly.
                // Normalize it once per incoming frame, then retain the
                // converted RenderImage for all subsequent paints.
