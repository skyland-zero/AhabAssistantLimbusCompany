...
export const STARLIGHT_ITEMS = [
  { index: 0, nameZh: "初始之星", nameEn: "Star of the Beginning", cost: 10, descZh: "初始经费增加，卡包/饰品展出+1，免费普通刷新" },
  { index: 1, nameZh: "积聚的星云", nameEn: "Cumulating Starcloud", cost: 10, descZh: "进阶经费利息+10%~30%，售卖饰品经费加成" },
  { index: 2, nameZh: "星际漫游", nameEn: "Interstellar Travel", cost: 20, descZh: "卡包出现+1，卡包刷新+2~4，未记录卡包等级提升" },
  { index: 3, nameZh: "流星", nameEn: "Star-shower", cost: 20, descZh: "初始经费+400~700，初始饰品可选择数+1" },
  { index: 4, nameZh: "双星商店", nameEn: "Binary Star-shop", cost: 30, descZh: "展出饰品+1，战斗经费+20%~40%，高阶饰品概率提升" },
  { index: 5, nameZh: "卫星商店", nameEn: "Moon Star-shop", cost: 30, descZh: "免费关键词刷新，进入第1层送1~3件1级饰品" },
  { index: 6, nameZh: "星云的宠爱", nameEn: "Favor of the Starcloud", cost: 40, descZh: "进入第1层人格等级+3，通关阶段人格等级提升" },
  { index: 7, nameZh: "星芒的引导", nameEn: "Guidance of the Starlight", cost: 40, descZh: "最大速度+2~3，拼点威力/伤害强化/守护提升" },
  { index: 8, nameZh: "偶然的彗星", nameEn: "Accidental Comet", cost: 50, descZh: "进商店赠送合成/售卖专用饰品，赠送对应关键词3/4级饰品" },
  { index: 9, nameZh: "全面的可能性", nameEn: "All-round Possibility", cost: 60, descZh: "开局自选3级饰品，获得残影饰品" },
] as const;
    ...
    try {
      ...
      if (parsed && typeof parsed === "object" && parsed.name) {
        setForm({
          ...
          mirrorConfig: parsed.mirrorConfig ? { ...createDefaultMirrorConfig(), ...parsed.mirrorConfig } : form.mirrorConfig,
        });
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1 flex flex-col min-h-0">
            ...
            <TabsList className="h-8 p-0.5 bg-muted/60">
              ...
              <TabsTrigger value="starlight" className="h-7 px-3 text-xs">
                ...
                {calculateStarlightCost > 0 && (
                  <Badge variant="secondary" className="ml-1.5 h-4 px-1 text-[10px] font-mono text-amber-500">
          ...
          <ScrollArea className="flex-1 min-h-0">
            ...
            <TabsContent value="basic" className="m-0 space-y-4">
                ...
                <div className="grid grid-cols-5 gap-2">
                  {TEAM_SYSTEMS.map((sys) => {
                    ...
                    return (
                      ...
                      >
                        <img src={sys.icon} alt={sys.id} className="size-5 shrink-0 object-contain" />
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 pt-1">
                <div className="p-3 rounded-lg border border-border/50 bg-card/20 space-y-2">
                  ...
                  <p className="text-[11px] text-muted-foreground">{t("teams.modal.useTeamCodeDesc")}</p>
                ...
                <div className="p-3 rounded-lg border border-border/50 bg-card/20 space-y-2">
                  ...
                  <p className="text-[11px] text-muted-foreground">{t("teams.modal.fixedUseDesc")}</p>
            ...
            <TabsContent value="shop" className="m-0 space-y-4">
              ...
              <div className="space-y-2 rounded-lg border border-border/60 bg-card/20 p-3.5">
                <div>
                  ...
                  <p className="text-[11px] text-muted-foreground">{t("teams.modal.discardSystemsDesc")}</p>
                </div>
                <div className="grid grid-cols-5 gap-2 pt-1">
                  {TEAM_SYSTEMS.map((sys) => {
                    ...
                    return (
                      ...
                      >
                        <img src={sys.icon} alt={sys.id} className="size-4 shrink-0 object-contain" />
                        <span className="truncate">{t(sys.labelKey)}</span>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="p-3 rounded-lg border border-border/50 bg-card/20 space-y-2">
                  ...
                  <div className="flex items-center justify-between text-xs">
                    ...
                    <Input
                      ...
                      onChange={(e) => updateMirror({ max_keyword_refresh: Number(e.target.value) || 0 })}
                  ...
                  <div className="flex items-center justify-between text-xs">
                    ...
                    <Input
                      ...
                      onChange={(e) => updateMirror({ max_normal_refresh: Number(e.target.value) || 0 })}
                      className="h-7 w-20 text-xs"
                    />
                  </div>
                </div>
                ...
                <div className="p-3 rounded-lg border border-border/50 bg-card/20 space-y-2">
                  <Label className="text-xs font-medium">{t("teams.modal.ignoreShopFloors")}</Label>
            ...
            <TabsContent value="combat" className="m-0 space-y-4">
              ...
              <div className="space-y-3 rounded-lg border border-border/60 bg-card/20 p-3.5">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-[11px] text-muted-foreground">{t("teams.modal.secondSystemDesc")}</p>
                  </div>
                  <Switch
                  ...
                  <div className="space-y-3 pt-2 border-t border-border/40">
                    <div className="grid grid-cols-2 gap-3">
                      <div className="space-y-1">
                        <Label className="text-[11px] text-muted-foreground">{t("teams.modal.secondSystemSelect")}</Label>
                        <Select
                      ...
                      <div className="space-y-1">
                        <Label className="text-[11px] text-muted-foreground">{t("teams.modal.secondSystemStartFloor")}</Label>
                        <Select
                          value={String(mirrorConfig.second_system_setting)}
                          onValueChange={(v) => updateMirror({ second_system_setting: Number(v) })}
                        >
                          <SelectTrigger className="h-8 text-xs">
                            <SelectValue />
                          </SelectTrigger>
                    ...
                    <div className="space-y-1.5">
                      <Label className="text-[11px] text-muted-foreground">{t("teams.modal.secondSystemActions")}</Label>
                      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                        ...
                        <label className="flex items-center gap-1.5 text-xs text-foreground cursor-pointer">
                          <Switch
                            ...
                            onCheckedChange={(v) => updateMirror({ second_system_select_reward: v })}
                          />
              ...
              <div className="space-y-3 rounded-lg border border-border/60 bg-card/20 p-3.5">
                <Label className="text-xs font-medium">{t("teams.modal.combatPreferences")}</Label>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <label className="flex items-center gap-2 text-xs text-foreground cursor-pointer">
                    <Switch
              ...
              <div className="space-y-3 rounded-lg border border-border/60 bg-card/20 p-3.5">
                <Label className="text-xs font-medium">{t("teams.modal.defenseStrategies")}</Label>
                <div className="space-y-2.5">
                  ...
                  <div className="space-y-1.5 pt-1">
                    <div className="flex items-center justify-between">
                      ...
                      {mirrorConfig.defense_for_solo && (
                        <Select
                          ...
                          onValueChange={(v) => updateMirror({ defense_for_solo_turns: Number(v) })}
                        >
                    ...
                    <p className="text-[11px] text-muted-foreground ml-8">{t("teams.modal.defenseForSoloDesc")}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="p-3.5 rounded-lg border border-border/60 bg-card/20 space-y-2">
                <div className="flex items-center justify-between">
                  <Label className="text-xs font-medium">{t("teams.modal.skillReplacement")}</Label>
                  <Switch
                ...
                {mirrorConfig.skill_replacement && (
                  <div className="pt-2 grid grid-cols-2 gap-3">
                    <div className="space-y-1">
                      <Label className="text-[11px] text-muted-foreground">{t("teams.modal.skillReplacementMode")}</Label>
                      <Select
            ...
            <TabsContent value="starlight" className="m-0 space-y-4">
              <div className="flex items-center justify-between p-3.5 rounded-lg border border-border/60 bg-card/20">
                <div>
                  ...
                  <p className="text-[11px] text-muted-foreground">{t("teams.modal.useStarlightDesc")}</p>
                </div>
                <Switch
              ...
              <div className="flex flex-wrap items-center justify-between gap-2 p-3 rounded-lg border border-border/50 bg-muted/30">
                ...
                <div className="flex items-center gap-2">
                  <Badge variant="outline" className="text-xs font-mono font-semibold bg-amber-500/10 text-amber-500 border-amber-500/30">
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-1.5">
                          <span className="text-xs font-semibold">{isEn ? item.nameEn : item.nameZh}</span>
            ...
            <TabsContent value="advanced" className="m-0 space-y-4">
              ...
              <div className="space-y-3 rounded-lg border border-border/60 bg-card/20 p-3.5">
                <div className="flex items-center justify-between">
                  <div>
                    <Label className="text-xs font-medium">{t("teams.modal.observeEgoGift")}</Label>
                    <p className="text-[11px] text-muted-foreground">{t("teams.modal.observeEgoGiftDesc")}</p>
                  </div>
                  <Switch
                  ...
                  <div className="space-y-2 pt-2 border-t border-border/40">
                    <div className="flex gap-2">
                      <Input
                        ...
                        onKeyDown={(e) => e.key === "Enter" && (e.preventDefault(), handleAddObserveGift())}
              ...
              <div className="p-3.5 rounded-lg border border-border/60 bg-card/20 flex items-center justify-between">
                <div>
                  <Label className="text-xs font-medium">{t("teams.modal.themePackWeight")}</Label>
                  <p className="text-[11px] text-muted-foreground">{t("teams.modal.themePackWeightDesc")}</p>
                </div>
                <Switch
              ...
              <div className="p-3.5 rounded-lg border border-border/60 bg-card/20 space-y-2">
                <div className="flex items-center justify-between">
                  <Label className="text-xs font-medium">{t("teams.modal.configImportExport")}</Label>
                  <Button
                    type="button"
                    variant="outline"
                    size="sm"
